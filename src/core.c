...
typedef struct {
    /* input parameters */
    CardId const* legal_cards;
    CardId legal_cards_len;
    GameInfo const* game_info;
    CardInfo const* card_info;
    /* additional options */
    uint8_t stop;
    float time_goal;
 ...
 *
 *  @param arg pointer to data structure holding all relevant data and return
 * data fields
...
#define NSIM 100
#define ELAPSED                                                                \
    ((finish.tv_sec - start.tv_sec) + 1.0e-9 * (finish.tv_nsec - start.tv_nsec))
    ...
    while (ELAPSED < arg->time_goal && arg->stop == 0) {
        ...
        for (uint8_t c = 0; c < arg->legal_cards_len; c++) {
            ...
            arg->results[c] += Simulate(&game_info_copy, arg->legal_cards[c],
                                        &arg->random_seed);
            arg->simulate_calls++;
        }
...
/** @brief gets best card. This is the main routine of the ai. It creates worker
 * threads and manages all central important stuff.
...
CardId GetBestCard(InputInfo const* input) {
    ...
    /* detect number of processors and launch as many thread. (threads are
     * capped at 16 though) */
    uint8_t thread_num;
    ...
    for (uint8_t thread_i = 0; thread_i < thread_num; thread_i++) {
        ...
        thread_args[thread_i].mc_sample_calls = 0ul;
        memset(&thread_args[thread_i].results, 0,
               sizeof(thread_args[thread_i].results));
    ...
    for (uint8_t c = 0; c < legal_cards_len; c++) {
        print_log(LOG_INFO, "%.2f points expected for %s",
                  (double)results[c] / (double)mc_sample_calls,
                  card_names_long[legal_cards[c] / 2]);
    ...
    print_log(
        LOG_INFO, "Simulated %lu games during %lu samplings in %hhux%0.3f s",
        simulate_calls, mc_sample_calls, thread_num, total_time / thread_num);
    print_log(LOG_INFO, "%.2f points expected for %s",
              (double)max_score / (double)mc_sample_calls,
              card_names_long[max_id / 2]);
    return max_id;
#undef MULTITHREADED
}
