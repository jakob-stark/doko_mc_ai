...
int analyze( const InputInfo* input, GameInfo* game_info, CardInfo* card_info ) {
    ...
    game_info->next = (4 + input->starting_player_id - input->computer_player_id)%4;
    ...
    if ( card_info->cards_left + input->computer_player_hand_len + input->played_cards_len != 48 ) {
        return -1;
    }

    if ( card_info->cards_left + input->computer_player_hand_len != game_info->cards_left ) {
        return -1;
    }
