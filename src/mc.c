...
#define RADIX 16
    ...
    for (uint8_t c = 0; c < card_info_copy.cards_left; c++) {
        ...
        for (uint8_t p = 0; p < 3; p++) {
            ...
            uint8_t s = card_info_copy.scores[c][p];
            switch (s) {
            case 0:
            case 3:
            case 4:
...
void mc_sample(GameInfo* dest, CardInfo const* card_info,
    ...
    uint8_t random[4];
    static uint8_t const score_to_slots[] = {0, 0, 0, 1, 2, 3};

    for (uint8_t c = 0; c < ci.cards_left; c++) {
        /* deal card at index c */
        random[0] = 0;
        /* block slots according to scores */
        for (uint8_t p = 0; p < 3; p++) {
            random[p + 1] =
                random[p] + ci.player_left[p] * score_to_slots[ci.scores[c][p]];
        }
        uint8_t r = random_uint8(random_state, random[3]);
        for (uint8_t p = 0; p < 3; p++) {
