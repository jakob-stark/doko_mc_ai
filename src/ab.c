...
hash_t Hash(const GameInfo* game_info) {
    hash_t result = 0;
    for ( uint8_t i = 0; i < sizeof(GameInfo)/2; i++ ) {
        result ^= ((const uint16_t*)game_info)[i];
    };
    return result;
}
...
Score GetFromTable(const GameInfo* game_info, Score alpha, Score beta) {
    hash_t hash = Hash(game_info);
    if ( Table[hash].score != 0 ) {
        if ( Table[hash].alpha <= alpha && beta <= Table[hash].beta ) {
            if ( memcmp(&(Table[hash].game_info), game_info, sizeof(GameInfo)) == 0 ) {
                hits++;
...
void StoreToTable(const GameInfo* game_info, Score score, Score alpha, Score beta) {
    hash_t hash = Hash(game_info);
    if ( Table[hash].score != 0 ) {
